defmodule SMPPEX.Protocol.OptionalFieldsBuilder do
  ...
  defp build_value_with_format(value, {:c_octet_string, {:max, size}}),
    do: Pack.c_octet_string(value, {:max, size})

  defp build_value_with_format(value, {:octet_string, size}) when is_integer(size),
    do: Pack.octet_string(value, size)
  ...
  defp build_value_without_format(value) when not is_binary(value),
    do: {:error, @invalid_unknown_tlv_value}
