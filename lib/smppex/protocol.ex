      ...
      case build_mandatory_fields(pdu, mandatory_specs) do
        {:ok, mandatory_bin} ->
          case build_optional_fields(pdu) do
            {:ok, optional_bin} ->
              {:ok, concat_pdu_binary_parts(header_bin, mandatory_bin, optional_bin)}

            {:error, error} ->
              {:error, {"Error building optional field part", error}}
          end

        {:error, error} ->
          {:error, {"Error building mandatory field part", error}}
      end
    else
